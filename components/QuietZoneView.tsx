...
import { PlayIcon, PauseIcon, ArrowPathIcon, MoonIcon, SparklesIcon, CheckCircleIcon, SpeakerWaveIcon } from './icons';
...
const ambientSounds = [
    ...
    { id: 'lofi', name: 'موسیقی لوفای', src: 'https://aistudio.google.com/static/demos/live-fm/lofi.mp3' },
    { id: 'rain', name: 'صدای باران', src: 'https://aistudio.google.com/static/demos/live-fm/rain.mp3' },
];
...
const QuietZoneView: React.FC<QuietZoneViewProps> = ({ goals, onUpdateGoals, onClose, addXp }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl z-50 flex items-center justify-center p-4 modal-backdrop" onClick={onClose}>
            ...
            <audio ref={audioRef} src={ambientSounds.find(s => s.id === ambientSound)?.src} loop />
            <div className="w-full max-w-lg flex flex-col items-center modal-panel" onClick={e => e.stopPropagation()}>
                ...
                <div className="w-full bg-slate-800/50 border border-blue-800/30 rounded-[var(--radius-card)] p-6">
                                ...
                                {ambientSounds.map(sound => (
                                    <button key={sound.id} onClick={() => {
                                        ...
                                        if (audioRef.current) {
                                            ...
                                            if (isActive && sound.id !== 'none') {
                                                audioRef.current.play();
                                            } else {
                                                audioRef.current.pause();
                                            }
                                        }
                                    }} className={`w-full text-right p-2 text-sm rounded-md flex justify-between items-center ${sound.id === ambientSound ? 'bg-blue-600 text-white' : 'hover:bg-slate-700'}`}>
